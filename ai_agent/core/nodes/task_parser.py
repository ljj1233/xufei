...
from ai_agent.core.state import GraphState, Task, TaskType, TaskPriority, TaskStatus
...
class TaskParser:
    ...
    def __call__(self, state: GraphState) -> GraphState:
        ...
        try:
            # 解析输入
            parsed_task = self._parse_input(state.input)
            
            # 创建任务
            task = self._create_task(parsed_task)
            
            # 更新状态
            state.task_state.current_task = task
            state.next_node = "strategy_decider"  # 设置下一个节点
            
        except Exception as e:
            state.error = f"Error parsing task: {str(e)}"
