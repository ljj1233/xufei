# ai_agent/tests/test_speech_analyzer.py
...
from ai_agent.analyzers.speech_analyzer import SpeechAnalyzer
from ai_agent.core.config import AgentConfig
from ai_agent.services.xunfei_service import XunFeiService
from ai_agent.analyzers.audio_feature_extractor import AudioFeatureExtractor
...
def speech_analyzer(mock_config):
    """创建语音分析器实例"""
    with patch("ai_agent.analyzers.speech_analyzer.XunFeiService") as mock_xunfei_service_cls:
        # 配置模拟的讯飞服务
...
def test_init(mock_config):
    ...
    # 测试讯飞服务初始化失败的情况
    with patch("ai_agent.analyzers.speech_analyzer.XunFeiService", side_effect=Exception("初始化失败")):  
        analyzer = SpeechAnalyzer(config=mock_config)
        assert analyzer.use_xunfei is False
        assert analyzer.xunfei_service is None
...
def test_extract_features(speech_analyzer):
    ...
    with patch("ai_agent.analyzers.speech_analyzer.AudioFeatureExtractor") as mock_extractor_cls:
        # 配置模拟的音频特征提取器
