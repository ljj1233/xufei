...
def mock_config():
    ...
    config = AgentConfig()
    # 设置测试配置
    config.set_config("face_detection_model", "haarcascade")
    config.set_config("frame_sample_rate", 5)
    config.set_config("visual", "use_xunfei_llm", True)
    return config
...
def visual_analyzer_no_llm(mock_config):
    ...
    # 修改配置，禁用讯飞LLM服务
    mock_config.set_config("visual", "use_xunfei_llm", False)
