...
def speech_analyzer_no_llm(mock_config):
    ...
    # 修改配置，禁用讯飞LLM服务
    mock_config.set_config("use_xunfei_llm", False)
...
async def test_analyze_with_llm_no_transcript(speech_analyzer_with_llm):
    ...
    # 配置模拟的音频读取
    with patch("agent.src.analyzers.speech.audio_feature_extractor.AudioFeatureExtractor") as mock_extractor_cls:
        mock_extractor_cls.read_audio_bytes = MagicMock(return_value=MOCK_AUDIO_BYTES)
