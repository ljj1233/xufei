...
class InterviewAgent:
    ...
    def __init__(self, config_path: Optional[str] = None, user_id: str = None, session_id: str = None):
        """初始化面试评测智能体
    ...
    def analyze(self, file_path: str, file_type: Optional[str] = None, 
                scenario: Optional[str] = None, params: Optional[Dict[str, Any]] = None,
                use_langgraph: bool = True) -> AnalysisResult:
        """分析面试
        ...
        Args:
            ...
            use_langgraph: 是否使用LangGraph框架进行分析，默认为True
        ...
        if use_langgraph:
            try:
                ...
                # 使用LangGraph智能体处理
                langgraph_result = self.langgraph_agent.process(input_data)
                
                # 如果LangGraph处理成功，直接返回结果
                if langgraph_result and "error" not in langgraph_result:
                    return AnalysisResult(langgraph_result)
                    
                # 如果LangGraph处理失败，回退到传统方法
                print(f"LangGraph分析失败，回退到传统方法: {langgraph_result.get('error', '未知错误')}")
            except Exception as e:
                print(f"LangGraph分析异常，回退到传统方法: {str(e)}")
        ...
        if use_langgraph:
            try:
                ...
                input_data = {
                    ...
                    "scenario": scenario,
                    "action": "start_real_time_analysis",
                    "timestamp": time.time()
                }
                
                # 使用LangGraph智能体处理
                self.langgraph_agent.process(input_data)
            except Exception as e:
                print(f"LangGraph初始化实时分析会话异常: {str(e)}")
            
        return True
    
    async def stop_real_time_analysis(self, session_id: str, use_langgraph: bool = True) -> bool:
        """停止实时分析
        ...
        if self.current_session_id == session_id:
            # 如果使用LangGraph框架，通知停止实时分析会话
            if use_langgraph:
                try:
                    # 准备输入数据
                    input_data = {
                        "session_id": session_id,
                        "action": "stop_real_time_analysis",
                        "timestamp": time.time()
                    }
                    ...
                    print(f"LangGraph停止实时分析会话异常: {str(e)}")
    ...
    async def analyze_audio_stream(self, session_id: str, audio_data: bytes, 
                                  timestamp: float = None, use_langgraph: bool = True) -> Dict[str, Any]:
        """分析音频流数据
                ...
                try:
                    ...
                    if langgraph_result and "error" not in langgraph_result:
                        ...
                        feedback = {
                            ...
                            "analysis": langgraph_result.get("analysis", {}),
                            "suggestions": langgraph_result.get("suggestions", [])
                        }
                        ...
                        return langgraph_result.get("analysis", {})
                        
                except Exception as e:
                    print(f"LangGraph音频流分析异常，回退到传统方法: {str(e)}")
        ...
        try:
            ...
            if use_langgraph:
                try:
                    ...
                    print(f"LangGraph视频帧分析异常，回退到传统方法: {str(e)}")
        ...
        if use_langgraph:
            try:
                ...
                input_data = {
                    ...
                    "action": "analyze_question_answer",
                    "timestamp": time.time()
                }
                ...
                # 如果LangGraph处理失败，回退到传统方法
                print(f"LangGraph问题回答分析失败，回退到传统方法: {langgraph_result.get('error', '未知错误')}")
            except Exception as e:
                print(f"LangGraph问题回答分析异常，回退到传统方法: {str(e)}")
